...
const AttendanceLogs: React.FC = () => {
  ...
  return (
    <div className="max-w-lg mx-auto py-4">
      <h2 className="text-xl font-semibold mb-4 text-gray-900" style={{ fontFamily: 'Montserrat' }}>Attendance Logs</h2>
      {logsLoading ? (
        ...
        <ul className="space-y-4">
          {logs.length > 0
            ? logs.slice(0, 5).map((entry) => (
                <li key={entry.name} className="bg-green-100 rounded-lg shadow-2xl p-4 flex flex-col" style={{ fontFamily: 'Rubik' }}>
                  <span className="font-medium text-blue-700">Check-in Time: {format(new Date(entry.time), 'PPpp')}</span>
            ...
            : lastFive.map((entry: AttendanceEntry) => (
                <li key={entry.id} className="bg-green-100 rounded-lg shadow-2xl p-4 flex flex-col" style={{ fontFamily: 'Rubik' }}>
